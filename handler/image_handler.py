...
def load_and_populate_workflow(workflow_name: str, params: Dict[str, Any]) -> (Dict[str, Any], str):
    ...
    for node_id, node_data in populated_workflow_dict.items():
        if isinstance(node_data.get("_meta", {}).get("title"), str) and "{{output_node_id}}" in node_data["_meta"][
            "title"]:
...
async def handle_generate_image(user_id: str, job_id: str, workflow_name: str, params: Dict[str, Any]) -> Optional[
    Dict[str, List[bytes]]]:
